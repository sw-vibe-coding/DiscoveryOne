# Step: source-loader-face-top-red
...
1. Add reg-rs case `d1_face_top_power` that runs
   `d1 face examples/power.d1 --face top`.
2. Capture the current RED output from the unsupported face path.
3. Do not implement top-face projection in this step.
4. Update `docs/status.md` to show top-face projection is the next RED
   M2 case.
...
`test: add top face projection baseline`
