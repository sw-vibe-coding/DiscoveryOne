# Step: source-loader-face-front-green

Implement the minimal front-face projection needed to make
`d1_face_front_power` green.
...
- Existing RED reg-rs case `d1_face_front_power`.

## Work

1. Add enough `d1-source` projection/rendering support to render the
   Front face of the Power fixture as a deterministic ASCII grid.
2. Wire `d1 face examples/power.d1 --face front` through the source
   loader.
3. Rebase `d1_face_front_power` to green.
4. Preserve `d1_normalize_roundtrip_power` and all M1 lexer baselines.
...
`test: green front face projection`
