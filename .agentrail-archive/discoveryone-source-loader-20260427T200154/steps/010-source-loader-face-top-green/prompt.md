# Step: source-loader-face-top-green

Implement the minimal top-face projection needed to make
`d1_face_top_power` green.
...
- Existing RED reg-rs case `d1_face_top_power`.

## Work

1. Add enough `d1-source` projection/rendering support to render the
   Top face of the Power fixture as a deterministic ASCII grid.
2. Wire `d1 face examples/power.d1 --face top` through the source
   loader.
3. Rebase `d1_face_top_power` to green.
4. Preserve `d1_normalize_roundtrip_power`, `d1_face_front_power`,
   `d1_face_left_power`, `d1_face_right_power`, and all M1 lexer
   baselines.
...
`test: green top face projection`
