...
- **Code:** Cargo workspace, placeholder library crates, Yew web
  ...
  aspect-tag, Z-layer, and comment lexer fixtures. Software
  Wrighter checklist metadata is clean.
- **Last meaningful commit:** Comment lexer green.
...
`d1_lex_comment`. The next RED step should add `d1_lex_neg_int`
without folding in percent literals. `sw-checklist` must remain
