...
- **Code:** Cargo workspace, placeholder library crates, Yew web
  ...
  `bottom` render Power facets. `d1_face_rear_power` captures
  the next RED baseline against the current unsupported-face
  path. Software Wrighter checklist metadata is clean.
- **Last meaningful commit:** Rear face projection RED baseline.
...
renders the Power bottom facet. `d1_face_rear_power` is RED and
records that `d1 face --face rear` currently reports an
