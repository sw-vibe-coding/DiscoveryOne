# Step: source-loader-face-front-red
...
- `docs/plan.md` M2 sequence: after normalize round-trip, add
  `d1 face --face F` baselines projecting Power.
...
1. Add reg-rs case `d1_face_front_power` that runs
   `d1 face examples/power.d1 --face front`.
2. Capture the current RED output from the scaffolded `d1 face` command.
3. Do not implement face projection in this step.
4. Update `docs/status.md` to show the front-face projection baseline is
   the next RED M2 case.
...
`test: add front face projection baseline`
