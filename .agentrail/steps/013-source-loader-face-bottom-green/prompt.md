# Step: source-loader-face-bottom-green

Implement the minimal bottom-face projection needed to make
`d1_face_bottom_power` green.
...
- Existing RED reg-rs case `d1_face_bottom_power`.

## Work

1. Add enough `d1-source` projection/rendering support to render the
   Bottom face of the Power fixture as a deterministic ASCII grid.
2. Wire `d1 face examples/power.d1 --face bottom` through the source
   loader.
3. Rebase `d1_face_bottom_power` to green.
4. Preserve `d1_normalize_roundtrip_power`, `d1_face_front_power`,
   `d1_face_left_power`, `d1_face_right_power`, `d1_face_top_power`,
   and all M1 lexer baselines.
...
`test: green bottom face projection`
