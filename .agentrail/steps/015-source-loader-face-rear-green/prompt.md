# Step: source-loader-face-rear-green

Implement the minimal rear-face projection needed to make
`d1_face_rear_power` green.
...
- Existing RED reg-rs case `d1_face_rear_power`.

## Work

1. Add enough `d1-source` projection/rendering support to render the
   Rear face of the Power fixture as a deterministic ASCII grid.
2. Wire `d1 face examples/power.d1 --face rear` through the source
   loader.
3. Rebase `d1_face_rear_power` to green.
4. Preserve `d1_normalize_roundtrip_power`, all five existing green
   face baselines, and all M1 lexer baselines.
...
`test: green rear face projection`
